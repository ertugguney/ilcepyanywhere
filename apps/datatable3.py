...
# all_options2 = {
#     'TR21-TRAKYA-(EDIRNE, KIRKLARELI, TEKIRDAG)':['EDIRNE','KIRKLARELI','TEKIRDAG'],
#     'TR10-ISTANBUL-(ISTANBUL)':['ISTANBUL'],
#     'TR22-GUNEY MARMARA-(BALIKESIR, CANAKKALE)':['BALIKESIR','CANAKKALE'],
#     'TR31-IZMIR-(IZMIR)':['IZMIR'],
#     'TR32-GUNEY EGE-(AYDIN, DENIZLI, MUGLA)':['AYDIN','DENIZLI','MUGLA'],
#     'TR33-ZAFER-(AFYONKARAHISAR, KUTAHYA, MANISA, USAK)':['AFYONKARAHISAR','KUTAHYA','MANISA','USAK'],
#     'TR41-BURSA, ESKISEHIR, BILECIK':['BILECIK','BURSA','ESKISEHIR'],
#     'TR42-DOGU MARMARA-(BOLU, DUZCE, KOCAELI, SAKARYA, YALOVA)':['BOLU','DUZCE','KOCAELI','SAKARYA','YALOVA'],
#     'TR51-ANKARA-(ANKARA)':['ANKARA'],
#     'TR52-MEVLANA-(KARAMAN, KONYA)':['KARAMAN','KONYA'],
#     'TR61-BATI KARADENIZ-(ANTALYA, BURDUR, ISPARTA)':['ANTALYA','BURDUR','ISPARTA'],
#     'TR62-CUKUROVA-(ADANA, MERSIN)':['ADANA','MERSIN'],
#     'TR63-DOGU AKDENIZ-(HATAY, KAHRAMANMARAS, OSMANIYE)':['HATAY','KAHRAMANMARAS','OSMANIYE'],
#     'TR71-AHILER-(AKSARAY, KIRIKKALE, KIRSEHIR, NIGDE, NEVSEHIR)':['AKSARAY','KIRIKKALE','KIRSEHIR','NEVSEHIR','NIGDE'],
#     'TR72-ORTA ANADOLU-(KAYSERI, SIVAS, YOZGAT)':['KAYSERI','SIVAS','YOZGAT'],
#     'TR81-BATI KARADENIZ-(BARTIN, KARABUK, ZONGULDAK)':['BARTIN','ZONGULDAK'],
#     'TR82-KUZEY ANADOLU-(CANKIRI, KASTAMONU, SINOP)':['CANKIRI','KASTAMONU','SINOP'],
#     'TR83-ORTA KARADENIZ-(AMASYA, CORUM, SAMSUN, TOKAT)':['AMASYA','CORUM','SAMSUN','TOKAT'],
#     'TR90-DOGU KARADENIZ-(ARTVIN, GIRESUN, GUMUSHANE, ORDU, RIZE, TRABZON)':['ARTVIN','GIRESUN','GUMUSHANE','ORDU','RIZE','TRABZON'],
#     'TRA1-KUZEY DOGU ANADOLU-(BAYBURT, ERZINCAN, ERZURUM)':['BAYBURT','ERZINCAN','ERZURUM'],
#     'TRA2-SERHAT-(AGRI, ARDAHAN, IGDIR, KARS)':['AGRI','ARDAHAN','IGDIR','KARS'],
#     'TRB1-FIRAT-(BINGOL, ELAZIG, MALATYA, TUNCELI)':['BINGOL','ELAZIG','MALATYA','TUNCELI'],
#     'TRB2-DOGU ANADOLU-(BITLIS, HAKKÂRI, MUS, VAN)':['BITLIS','HAKKARI','MUS','VAN'],
#     'TRC1-IPEKYOLU-(ADIYAMAN, GAZIANTEP, KILIS)':['ADIYAMAN','GAZIANTEP','KILIS'],
#     'TRC2-KARACADAG-(DIYARBAKIR, SANLIURFA)':['DIYARBAKIR','SANLIURFA'],
#     'TRC3-DICLE-(BATMAN, MARDIN, SIRNAK, SIIRT)':['BATMAN','MARDIN','SIIRT','SIRNAK'],
#     }
...
layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col([
            html.H5('Ulusal İlçe Verileri-İki Farklı Veriye göre Saçılım Grafiği',
        ...
        dbc.Col([
            ...
            dcc.Dropdown(
                id='countries-dropdown13',
                options=[{'label': k, 'value': k} for k in all_options.keys()],
                value='Nüfus',clearable=False,
                persistence=True, persistence_type='session',
                ),

            dcc.Dropdown(id='cities-dropdown13', value='Nüfus',clearable=False,
                persistence=True, persistence_type='session',),

            dcc.Dropdown(id='landmarks-dropdown13',clearable=False,
                persistence=True, persistence_type='session',),
            html.H6(id='display-selected-values13',
        ...
        dbc.Col([
            html.H5("Y Ekseni",
                    className='text-start text-primary mr-4'),
            dcc.Dropdown(
                id='countries-dropdown3',
                options=[{'label': k, 'value': k} for k in all_options.keys()],
                value='Nüfus',clearable=False,
                persistence=True, persistence_type='session',
                ),

            dcc.Dropdown(id='cities-dropdown3', value='Nüfus',clearable=False,
                persistence=True, persistence_type='session',),

            dcc.Dropdown(id='landmarks-dropdown3',clearable=False,
                persistence=True, persistence_type='session',),

            html.H6(id='display-selected-values3',
                    className='text-start text-danger font-weight-bold mt-3 ml-1'),
        ],
            xs=12, sm=12, md=12, lg=6, xl=6,
        ),
    ...
    #     )
    # ], align="center",style={'marginTop': 260}),
    ...
    dbc.Row([
        ...
        dbc.Col([
            html.H5("Saçılım Grafiği",
                    className='text-start text-primary mr-4'),
            dcc.Graph(id='polinom_graph3', figure={}),
        ],
           xs=12, sm=12, md=12, lg=12, xl=12
        ),
...
#         dbc.Col([
#             html.H5("Bölge ve İl Seçimi (Karşılaştırmalı)",
#                     className='text-start text-primary mr-4'),
#             dcc.Dropdown(
#                 id='countries-dropdown2',
#                 options=[{'label': k, 'value': k} for k in all_options2.keys()],
...
#     ], align="start"),
    ...
    dbc.Row([
        ...
        dbc.Col([
            html.Button("Tüm veritabanını CSV olarak indir", id="btn_csv13"),
            dcc.Download(id="download-dataframe-csv13"),
        ],
            xs=12, sm=12, md=12, lg=3, xl=3
        ),

        dbc.Col([
            html.Button("Yerleşim yerleri belgesini CSV olarak indir", id="btn_csv23"),
            dcc.Download(id="download-dataframe-csv23"),
        ],
            xs=12, sm=12, md=12, lg=3, xl=3
        ),

        dbc.Col([
            html.Button("Metaveri belgesini CSV olarak indir", id="btn_csv33"),
            dcc.Download(id="download-dataframe-csv33"),
        ],
            xs=12, sm=12, md=12, lg=3, xl=3
        ),
...
@app.callback([
    dash.dependencies.Output('cities-dropdown3', 'options'),
    dash.dependencies.Output('cities-dropdown3', 'value'),],
    [dash.dependencies.Input('countries-dropdown3', 'value'),])
...
@app.callback([dash.dependencies.Output('landmarks-dropdown3', 'options'),
    dash.dependencies.Output('landmarks-dropdown3', 'value'),],
    [dash.dependencies.Input('countries-dropdown3', 'value'),
    dash.dependencies.Input('cities-dropdown3', 'value'),])
...
# Alternatif çözüm denemesi: 3 dropdown'ın 2 callback'te gösterilmesi
@app.callback([
    dash.dependencies.Output('cities-dropdown13', 'options'),
    dash.dependencies.Output('cities-dropdown13', 'value'),],
    [dash.dependencies.Input('countries-dropdown13', 'value'),])
def set_landmarks_value(selected_country1):
    if selected_country1 is None:
        raise PreventUpdate
    else:
        return ([{'label': i, 'value': i} for i in all_options[selected_country1]],
            [{'label': i, 'value': i} for i in all_options[selected_country1]][0]['value'])
@app.callback([dash.dependencies.Output('landmarks-dropdown13', 'options'),
    dash.dependencies.Output('landmarks-dropdown13', 'value'),],
    [dash.dependencies.Input('countries-dropdown13', 'value'),
    dash.dependencies.Input('cities-dropdown13', 'value'),])
def set_landmarks_value(selected_country1,selected_city1):
    if (selected_country1 or selected_city1) is None:
        raise PreventUpdate
    else:
        return ([{'label': y, 'value': y} for y in all_options[selected_country1][selected_city1]],
            [{'label': y, 'value': y} for y in all_options[selected_country1][selected_city1]][0]['value'])
...
# def set_cities_options(selected_country2):
#     if selected_country2 is None:
#         raise PreventUpdate
#     else:
#         if type(selected_country2) == 'str':
#             counties_of_states=[{'label': i, 'value': i} for i in all_options2[selected_country2]]
#         else:
#             counties_of_states=[{'label': i, 'value': i} for country in selected_country2 for i in all_options2[country]]
#         values_selected=[counties_of_states[0]['value']]
#         return counties_of_states, values_selected

#**************************************************

@app.callback(
    dash.dependencies.Output("download-dataframe-csv13", "data"),
    dash.dependencies.Input("btn_csv13", "n_clicks"),
...
@app.callback(
    dash.dependencies.Output("download-dataframe-csv23", "data"),
    dash.dependencies.Input("btn_csv23", "n_clicks"),
    prevent_initial_call=True,)
def func(n_clicks):
    if n_clicks is None:
        raise PreventUpdate
...
@app.callback(
    dash.dependencies.Output("download-dataframe-csv33", "data"),
    dash.dependencies.Input("btn_csv33", "n_clicks"),
    prevent_initial_call=True,)
def func(n_clicks):
    if n_clicks is None:
        raise PreventUpdate
...
@app.callback(
    [dash.dependencies.Output('display-selected-values3', 'children'),
     dash.dependencies.Output('display-selected-values13', 'children'),
    ...
    #  dash.dependencies.Output('tree_map','figure'),
     dash.dependencies.Output('polinom_graph3','figure'),
    ...
    [dash.dependencies.Input('countries-dropdown3', 'value'),
     dash.dependencies.Input('cities-dropdown3', 'value'),
     dash.dependencies.Input('landmarks-dropdown3', 'value'),
     dash.dependencies.Input('countries-dropdown13', 'value'),
     dash.dependencies.Input('cities-dropdown13', 'value'),
     dash.dependencies.Input('landmarks-dropdown13', 'value'),
    #  dash.dependencies.Input('countries-dropdown2', 'value'),
    #  dash.dependencies.Input('cities-dropdown2', 'value'),
    #  dash.dependencies.Input('landmarks-dropdown2', 'value'),   #3'lü Bölge, İl, İlçe Dropdown'da gerekliydi
     dash.dependencies.Input('slider_year3','value'),
     ],
     )

def update_graph(selected_country,selected_city,selected_landmark,selected_country1,selected_city1,selected_landmark1,
    # selected_country2,selected_city2,
    slider_year):
    if (selected_country or selected_city or selected_landmark or selected_country1 or selected_city1 or selected_landmark1 or slider_year) is None:
        raise PreventUpdate
    else:
        dff = df.copy()
        ...
        # dff_ilce = dff_ilce[(dff_ilce["anabaslik"]==selected_country)&(dff_ilce["ortabaslik"]==selected_city)&(dff_ilce["baslik"]==selected_landmark)&(dff_ilce["Bölge (Düzey-2)"].isin(list(selected_country2)))&(dff_ilce["İl Adı"].isin(list(selected_city2)))].sort_values(["Yıl","deger"],ascending=[False,False])
        ...
        dffYıl1=df.copy()
        dffYıl1 = dffYıl1[(dffYıl1["anabaslik"]==selected_country1)&(dffYıl1["ortabaslik"]==selected_city1)&(dffYıl1["baslik"]==selected_landmark1)]
        ilkYıl1=np.min(dffYıl1['Yıl'])
        sonYıl1=np.max(dffYıl1['Yıl'])
        ...
        if np.sum(dff['deger'])==0:
            ...
            fig7 = go.Figure().add_annotation(x=2, y=2,text=u'Y ekseni başlığında seçilen yıl için veri bulunmamaktadır, {}-{} yılları arasında veri bulunmaktadır'.format(ilkYıl,sonYıl),font=dict(family="sans serif",size=25,color="red"),showarrow=False,yshift=10)
            ...
            return u'Bu başlıkta seçilen yıl için veri bulunmamaktadır, {}-{} yılları arasında veri bulunmaktadır'.format(ilkYıl,sonYıl),u'Bu başlıkta {}-{} yılları arasında veri bulunmaktadır'.format(ilkYıl1,sonYıl1),fig7,
            # ,fig8,fig9,fig10,fig11,fig4,fig5,fig6,
        elif np.sum(dff1['deger1'])==0:
            # fig4 = go.Figure().add_annotation(x=2, y=2,text=u'Bu başlıkta seçilen yıl için veri bulunmamaktadır, {}-{} yılları arasında veri bulunmaktadır'.format(ilkYıl,sonYıl),font=dict(family="sans serif",size=25,color="red"),showarrow=False,yshift=10),
            # fig5 = go.Figure().add_annotation(x=2, y=2,text=u'Bu başlıkta seçilen yıl için veri bulunmamaktadır, {}-{} yılları arasında veri bulunmaktadır'.format(ilkYıl,sonYıl),font=dict(family="sans serif",size=25,color="red"),showarrow=False,yshift=10)
            # fig6 = go.Figure().add_annotation(x=2, y=2,text=u'Bu başlıkta seçilen yıl için veri bulunmamaktadır, {}-{} yılları arasında veri bulunmaktadır'.format(ilkYıl,sonYıl),font=dict(family="sans serif",size=25,color="red"),showarrow=False,yshift=10),
            fig8 = go.Figure().add_annotation(x=2, y=2,text=u'X ekseni başlığında seçilen yıl için veri bulunmamaktadır, {}-{} yılları arasında veri bulunmaktadır'.format(ilkYıl1,sonYıl1),font=dict(family="sans serif",size=25,color="red"),showarrow=False,yshift=10)
            # fig8 = go.Figure().add_annotation(x=2, y=2,text=u'Bu başlıkta seçilen yıl için veri bulunmamaktadır, {}-{} yılları arasında veri bulunmaktadır'.format(ilkYıl,sonYıl),font=dict(family="sans serif",size=25,color="red"),showarrow=False,yshift=10),
            # fig9 = go.Figure().add_annotation(x=2, y=2,text=u'Bu başlıkta seçilen yıl için veri bulunmamaktadır, {}-{} yılları arasında veri bulunmaktadır'.format(ilkYıl,sonYıl),font=dict(family="sans serif",size=25,color="red"),showarrow=False,yshift=10)
            # fig10 = go.Figure().add_annotation(x=2, y=2,text=u'Bu başlıkta seçilen yıl için veri bulunmamaktadır, {}-{} yılları arasında veri bulunmaktadır'.format(ilkYıl,sonYıl),font=dict(family="sans serif",size=25,color="red"),showarrow=False,yshift=10),
            # fig11 = go.Figure().add_annotation(x=2, y=2,text=u'Bu başlıkta seçilen yıl için veri bulunmamaktadır, {}-{} yılları arasında veri bulunmaktadır'.format(ilkYıl,sonYıl),font=dict(family="sans serif",size=25,color="red"),showarrow=False,yshift=10),
            return u'Bu başlıkta {}-{} yılları arasında veri bulunmaktadır'.format(ilkYıl,sonYıl),u'Bu başlıkta seçilen yıl için veri bulunmamaktadır, {}-{} yılları arasında veri bulunmaktadır'.format(ilkYıl1,sonYıl1),fig8
            # ,fig8,fig9,fig10,fig11,fig4,fig5,fig6,
        else:
            ...
            # dff["Türkiye"] = "Türkiye"
            # dff2=dff.copy()
            # dff2 = dff.append({'İlçe Adı': 'Türkiye', 'deger': dff2['deger'].sum(),'Türkiye':''}, ignore_index=True)
            ...
            # figure4.update_layout(legend_traceorder="normal")
            ...
            ilkYıl,sonYıl),u'Bu başlıkta {}-{} yılları arasında veri bulunmaktadır'.format(
            ilkYıl1,sonYıl1),figure3,
                    # figure4,figure5,figure6,figure7,fig,figure1,figure2,
