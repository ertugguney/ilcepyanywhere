...
layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col([
            html.H5('Ulusal İlçe Verileri-Harita Gösterimi',
        ...
        dbc.Col([
            ...
            dcc.Dropdown(
                id='countries-dropdown11',clearable=False,
                persistence=True, persistence_type='session',
                options=[{'label': k, 'value': k} for k in all_options.keys()],
                value='Nüfus'
                ),

            dcc.Dropdown(id='cities-dropdown11', value='Nüfus',clearable=False,
                persistence=True, persistence_type='session',),

            dcc.Dropdown(id='landmarks-dropdown11',clearable=False,
                persistence=True, persistence_type='session',),

            html.H6(id='display-selected-values11',
    ...
    dbc.Row([
        dbc.Col([
            dcc.Graph(id='my_bee_map11', figure={},
                      ),
        ],
...
#     html.Br(),
    ...
    dbc.Row([
        ...
        dbc.Col([
            html.Button("Tüm veritabanını CSV olarak indir", id="btn_csv111"),
            dcc.Download(id="download-dataframe-csv111"),
        ],
            xs=12, sm=12, md=12, lg=3, xl=3
        ),

        dbc.Col([
            html.Button("Yerleşim yerleri belgesini CSV olarak indir", id="btn_csv211"),
            dcc.Download(id="download-dataframe-csv211"),
        ],
            xs=12, sm=12, md=12, lg=3, xl=3
        ),

        dbc.Col([
            html.Button("Metaveri belgesini CSV olarak indir", id="btn_csv311"),
            dcc.Download(id="download-dataframe-csv311"),
        ],
            xs=12, sm=12, md=12, lg=3, xl=3
        ),
...
@app.callback([
    dash.dependencies.Output('cities-dropdown11', 'options'),
    dash.dependencies.Output('cities-dropdown11', 'value'),],
    [dash.dependencies.Input('countries-dropdown11', 'value'),])
...
@app.callback([dash.dependencies.Output('landmarks-dropdown11', 'options'),
    dash.dependencies.Output('landmarks-dropdown11', 'value'),],
    [dash.dependencies.Input('countries-dropdown11', 'value'),
    dash.dependencies.Input('cities-dropdown11', 'value'),])
...
@app.callback(
    dash.dependencies.Output("download-dataframe-csv111", "data"),
    dash.dependencies.Input("btn_csv111", "n_clicks"),
...
@app.callback(
    dash.dependencies.Output("download-dataframe-csv211", "data"),
    dash.dependencies.Input("btn_csv211", "n_clicks"),
    prevent_initial_call=True,)
def func(n_clicks):
    if n_clicks is None:
        raise PreventUpdate
...
@app.callback(
    dash.dependencies.Output("download-dataframe-csv311", "data"),
    dash.dependencies.Input("btn_csv311", "n_clicks"),
    prevent_initial_call=True,)
def func(n_clicks):
    if n_clicks is None:
        raise PreventUpdate
...
@app.callback(
    [dash.dependencies.Output('display-selected-values11', 'children'),
     dash.dependencies.Output('my_bee_map11', 'figure'),
    #  dash.dependencies.Output('bar_graph1','figure'),
    #  dash.dependencies.Output('tree_map1','figure'),
    ...
    [dash.dependencies.Input('countries-dropdown11', 'value'),
     dash.dependencies.Input('cities-dropdown11', 'value'),
     dash.dependencies.Input('landmarks-dropdown11', 'value'),
    #  dash.dependencies.Input('countries-dropdown1', 'value'),
    #  dash.dependencies.Input('cities-dropdown1', 'value'),
    ...
    #  dash.dependencies.Input('landmarks-dropdown2', 'value'),   #3'lü Bölge, İl, İlçe Dropdown'da gerekliydi
     dash.dependencies.Input('slider_year11','value'),
     ],
     )
...
def update_graph(selected_country,selected_city,selected_landmark,
    ...
    if (selected_country or selected_city or selected_landmark or slider_year) is None:
        ...
        if np.sum(dff['deger'])==0:
            ...
            return u'Bu başlıkta seçilen yıl için veri bulunmamaktadır, {}-{} yılları arasında veri bulunmaktadır'.format(ilkYıl,sonYıl),fig4,
