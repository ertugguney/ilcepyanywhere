...
# https://media.githubusercontent.com/media/ertugguney/datasets/main/ilceData_04.12.21_Il_Ilcesiz.csv
...
layout = dbc.Container([
    # dbc.Row([
    #     dbc.Col([
    #         html.H6('Grafik Gösterimi için Lütfen Bölge Seçiniz',
    #             className='text-start text-danger font-weight-bold'),
    #     ],xl=12)
    # ]),
    dbc.Row([
        dbc.Col([
            html.H5('Bölgesel İlçe Verileri (Karşılastırmalı)',
                className='text-center text-primary font-weight-bold'),
        ],xl=12),

        dbc.Col([
            html.H6('Grafik Gösterimi için Lütfen Bölge Seçiniz',
                className='text-start text-danger font-weight-bold ml-1'),
        ],xl=12),
        ...
        dbc.Col([
            ...
            dcc.Dropdown(id='cities-dropdown22',multi=True,
                # value='EDIRNE',
                placeholder="İl Seçiniz",clearable=False,
                persistence=True, persistence_type='session',
                # searchable=False
                # clearable=False,
                ),
            html.H6(id='display-selected-values2',
        ...
        dbc.Col([
            ...
            dcc.Dropdown(id='cities-dropdown2', value='Nüfus',clearable=False,
                persistence=True, persistence_type='session',),

            dcc.Dropdown(id='landmarks-dropdown2',clearable=False,
                persistence=True, persistence_type='session',),
        ],
            xs=12, sm=12, md=12, lg=6, xl=6
        ),
...
#     ),
    ...
    dbc.Row([
        dbc.Col([
            html.H5("İlçe Trend Grafiği",
                    className='text-start text-primary mr-4'),
            dcc.Graph(id='trend_ilce2', figure={}),
        ],
           xs=12, sm=12, md=12, lg=12, xl=12
        ),
    ], align="center",
    ...
    dbc.Row([
        ...
        dbc.Col([
            html.H5("İl Bar Grafiği",
                    className='text-start text-primary mr-4'),
            dcc.Graph(id='bar_graph_il2', figure={}),
        ],
           xs=12, sm=12, md=12, lg=12, xl=12
        ),
    ...
    dbc.Row([
        ...
        dbc.Col([
            html.H5("Bölgesel Bar Grafiği",
                    className='text-start text-primary mr-4'),
            dcc.Graph(id='bar_graph_bolge2', figure={}),
        ],
           xs=12, sm=12, md=12, lg=12, xl=12
        ),
    ...
    dbc.Row([
        ...
        dbc.Col([
            html.H5("Bölgesel Ağaç Haritası",
                    className='text-start text-primary mr-4'),
            dcc.Graph(id='tree_map_bolgesel2', figure={},
                      ),
        ],
           xs=12, sm=12, md=12, lg=12, xl=12
        )
    ...
    dbc.Row([
        ...
        dbc.Col([
            html.Button("Tüm veritabanını CSV olarak indir", id="btn_csv12"),
            dcc.Download(id="download-dataframe-csv12"),
        ],
            xs=12, sm=12, md=12, lg=3, xl=3
        ),

        dbc.Col([
            html.Button("Yerleşim yerleri belgesini CSV olarak indir", id="btn_csv22"),
            dcc.Download(id="download-dataframe-csv22"),
        ],
            xs=12, sm=12, md=12, lg=3, xl=3
        ),

        dbc.Col([
            html.Button("Metaveri belgesini CSV olarak indir", id="btn_csv32"),
            dcc.Download(id="download-dataframe-csv32"),
        ],
            xs=12, sm=12, md=12, lg=3, xl=3
        ),
...
@app.callback([dash.dependencies.Output('landmarks-dropdown2', 'options'),
...
# Çalışan Alternatif 1
@app.callback([
    dash.dependencies.Output('cities-dropdown22', 'options'),
    dash.dependencies.Output('cities-dropdown22', 'value')],
    [dash.dependencies.Input('countries-dropdown22', 'value')])
...
def set_cities_options(selected_country2):
    if selected_country2 is None:
        ...
        if type(selected_country2) == 'str':
            ...
            counties_of_states=[{'label': y, 'value': y} for country in selected_country2 for y in all_options2[country]]
...
@app.callback(
    dash.dependencies.Output("download-dataframe-csv12", "data"),
    dash.dependencies.Input("btn_csv12", "n_clicks"),
...
@app.callback(
    dash.dependencies.Output("download-dataframe-csv22", "data"),
    dash.dependencies.Input("btn_csv22", "n_clicks"),
    prevent_initial_call=True,)
def func(n_clicks):
    if n_clicks is None:
        raise PreventUpdate
...
@app.callback(
    dash.dependencies.Output("download-dataframe-csv32", "data"),
    dash.dependencies.Input("btn_csv32", "n_clicks"),
    prevent_initial_call=True,)
def func(n_clicks):
    if n_clicks is None:
        raise PreventUpdate
...
@app.callback(
    [dash.dependencies.Output('display-selected-values2', 'children'),
    ...
    #  dash.dependencies.Output('polinom_graph','figure'),
     dash.dependencies.Output('trend_ilce2','figure'),
     dash.dependencies.Output('bar_graph_il2','figure'),
     dash.dependencies.Output('bar_graph_bolge2','figure'),
     dash.dependencies.Output('tree_map_bolgesel2','figure'),
     ],
    [dash.dependencies.Input('countries-dropdown2', 'value'),
     dash.dependencies.Input('cities-dropdown2', 'value'),
     dash.dependencies.Input('landmarks-dropdown2', 'value'),
    #  dash.dependencies.Input('countries-dropdown1', 'value'),
    #  dash.dependencies.Input('cities-dropdown1', 'value'),
    #  dash.dependencies.Input('landmarks-dropdown1', 'value'),
     dash.dependencies.Input('countries-dropdown22', 'value'),
     dash.dependencies.Input('cities-dropdown22', 'value'),
    #  dash.dependencies.Input('landmarks-dropdown2', 'value'),   #3'lü Bölge, İl, İlçe Dropdown'da gerekliydi
     dash.dependencies.Input('slider_year2','value'),
     ],
     )

def update_graph(selected_country,selected_city,selected_landmark,selected_country2,selected_city2,slider_year):
    # selected_country1,selected_city1,selected_landmark1,
    if (selected_country or selected_city or selected_landmark or selected_country2 or selected_city2 or slider_year) is None:
        raise PreventUpdate
    else:
        dff = df.copy()
        ...
        if np.sum(dff['deger'])==0:
            ...
            return u'Bu başlıkta seçilen yıl için veri bulunmamaktadır, {}-{} yılları arasında veri bulunmaktadır'.format(ilkYıl,sonYıl),fig8,fig9,fig10,fig11
                # fig4,fig5,fig6,fig7,
        else:
            ...
            # if np.sum(dff_bolge['deger'])==0:
            #     if np.sum(dff_ilce['deger'])==0:
            #         figure4={"layout": {"xaxis": {"visible": True},"yaxis": {"visible": True},"annotations": [{
            #                         "text": "Bu başlıkta seçilen il/iller için veri bulunmamaktadır",
            #                         "xref": "paper",
            #                         "yref": "paper",
            #                         "showarrow": False,
            #                         "font": {"size": 25, "family":"sans serif","color":"red"}}]}}
            #     else:
            ...
            #     figure5 = {"layout": {"xaxis": {"visible": True},"yaxis": {"visible": True},"annotations": [{
            #                     "text": "Bu başlıkta seçilen yılda bu il/iller için veri bulunmamaktadır",
            #                     "xref": "paper",
            #                     "yref": "paper",
            #                     "showarrow": False,
            #                     "font": {"size": 25, "family":"sans serif","color":"red"}}]}}
            #     figure6 = {"layout": {"xaxis": {"visible": True},"yaxis": {"visible": True},"annotations": [{
            #                     "text": "Bu başlıkta seçilen yılda bu Bölge/Bölgeler için veri bulunmamaktadır",
            #                     "xref": "paper",
            #                     "yref": "paper",
            #                     "showarrow": False,
            #                     "font": {"size": 25, "family":"sans serif","color":"red"}}]}}
            # elif np.sum(dff_il['deger'])==0:
            #     if np.sum(dff_ilce['deger'])==0:
            #         figure4={"layout": {"xaxis": {"visible": True},"yaxis": {"visible": True},"annotations": [{
            #                         "text": "Bu başlıkta seçilen il/iller için veri bulunmamaktadır",
            #                         "xref": "paper",
            ...
            ilkYıl,sonYıl),figure4,figure5,figure6,figure7,
            # fig,figure1,figure2,figure3,
